...
const ValueProposition = () => {
  ...
  const items = [
    {
      title: "Strategic LinkedIn visibility that converts",
      desc: "Get recognized by your name while attracting 4x more qualified discovery calls than generic marketing.",
    },
    {
      title: "Content that drives real leads",
      desc: "Set yourself as a thought leader on LinkedIn and generate 3x the inbound leads without losing long hours to content creation.",
    },
    {
      title: "Messaging that magnetizes prospects",
      desc: "Stand out from other coaches and consultants on LinkedIn with content that converts 67% better than industry averages.",
    },
    {
      title: "Predictable pipeline that delivers",
      desc: "Attract 12-15 qualified leads monthly, even when referrals dry up and networking events disappoint.",
    },
    {
      title: "Consistent presence without burnout",
      desc: "Keep your LinkedIn personal brand growing without sacrificing precious client delivery time.",
    },
  ];
    ...
    const handleScroll = () => {
      ...
      cardsRef.current.forEach((card, index) => {
        ...
        const cardProgress = Math.max(
          0,
          Math.min(1, scrollProgress * items.length - index)
        );
        ...
        if (isActive) {
          ...
          card.style.transform = `translateY(${
            isMobile ? 50 : 100
          }px) scale(0.9) rotate(${rotation}deg)`;
  ...
  return (
    ...
    >
      <div className="w-[90%] md:w-[80%] mx-auto max-w-7xl">
        ...
        <div
          ref={stickyRef}
          className={`h-screen flex items-center justify-center z-20 ${
            isMobile ? "pb-36" : "pb-0"
          } pointer-events-none`}
        ...
        >
          <div
            ...
            } w-full max-w-7xl px-4 ${isMobile ? "gap-6" : "gap-12"}`}
          >
            {/* Left Title Section */}
            <div
              className={`${
                isMobile ? "w-full text-center" : "flex-1 max-w-lg"
              }`}
            >
              ...
              >
                How we make you the obvious choice
                <span className="text-orange-500 block">in your industry</span>
              </h2>
              ...
              >
                ...
                <div
                  className={`${
                    isMobile ? "flex-1 max-w-32" : "flex-1"
                  } bg-white/20 h-1 rounded-full overflow-hidden`}
                >
                  <div
                    className="bg-orange-500 h-full transition-all duration-300 ease-out"
                    style={{
                      width: `${
                        ((parseInt(numberRef.current?.textContent || "1") - 1) /
                          (items.length - 1)) *
                        100
                      }%`,
                    }}
            ...
            >
              <div
                className={`relative w-full ${
                  isMobile ? "max-w-sm h-64" : "max-w-xl h-96"
                } flex items-center justify-center`}
              >
                {items.map((item, i) => (
                  <div
                  ...
                  >
                    <div
                      className={`bg-white border border-gray-200 rounded-xl ${
                        isMobile ? "p-4" : "p-6"
                      } w-full h-full shadow-2xl flex flex-col items-left justify-center text-left`}
                    >
        ...
        <div
          className={`absolute ${
            isMobile ? "bottom-8" : "bottom-12 lg:bottom-8"
          } left-1/2 transform -translate-x-1/2 text-center pt-4 lg:pt-16 z-30`}
        >
          <button
