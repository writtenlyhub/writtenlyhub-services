...
const WhyChooseMe = () => {
  ...
  const [minHeightPx, setMinHeightPx] = useState("auto")
  ...
  // compute a tight minHeight based on the actual rendered card height
  useEffect(() => {
    const computeMinHeight = () => {
      if (typeof window === "undefined") return
      // Wait a frame to ensure refs are attached and layout is stable
      requestAnimationFrame(() => {
        const viewport = window.innerHeight
        const stickyEl = stickyRef.current
        const firstCard = cardsRef.current && cardsRef.current[0]
        // fallback guesses if DOM not ready yet
        const fallbackCardHeight = isMobile ? 224 : 288

        const cardHeight = firstCard ? firstCard.offsetHeight : fallbackCardHeight
        const stickyHeight = stickyEl ? stickyEl.offsetHeight : viewport

        // small overlap so last card sits nicely before CTA.
        const overlapAdjustment = Math.round(cardHeight * 0.12)

        // Slightly reduce the previous tightening so a little space remains.
        const extraTighten = 40 // px; smaller value keeps a small gap below cards
        const computed = stickyHeight + (items.length - 1) * cardHeight - overlapAdjustment - extraTighten

        setMinHeightPx(`${Math.max(computed, viewport + 10)}px`)
      })
    }

    // compute on mount and resize — also recompute shortly after mount to catch render changes
    computeMinHeight()
    const t = setTimeout(computeMinHeight, 120)
  ...
  useEffect(() => {
    let currentIndex = 0
    const cardRotations = [-2, 1.5, -1, 2, 0.5]
    ...
    const handleScroll = () => {
      ...
      cardsRef.current.forEach((card, index) => {
        ...
        if (isActive) {
          ...
          const yOffset = (1 - cardProgress) * (isMobile ? 20 : 36)
          ...
          for (let i = 0; i < index; i++) {
            ...
            if (prev) {
              const stackOffset = (index - i) * (isMobile ? 3 : 6)
          ...
          const rotation = isMobile ? 0 : cardRotations[index % cardRotations.length]
          card.style.transform = `translateY(${isMobile ? 40 : 80}px) scale(0.9) rotate(${rotation}deg)`
  ...
  return (
    <>
      <section
        id="WhyChooseMe"
        ref={containerRef}
        className="bg-[#022150] py-12 md:py-20 font-montserrat overflow-hidden relative"
      ...
      >
        <div className="w-[90%] md:w-[80%] mx-auto max-w-7xl">
          ...
          >
            <div className="flex h-full flex-col items-center justify-start gap-4 md:gap-8 px-4 pt-4 md:pt-8">
              <header className="w-full max-w-3xl text-center space-y-3 md:space-y-5">
              ...
              <div className="w-full flex items-center justify-center">
                ...
                <div className="relative w-full max-w-sm md:max-w-xl h-56 md:h-72">
                  {items.map((item, i) => (
                    <div
                      key={i}
                      ref={addToRefs}
                    ...
                    >
                      <article
                        className="pointer-events-auto bg-gradient-to-br from-white via-white to-slate-50 rounded-2xl md:rounded-3xl shadow-2xl border border-white/20 w-full h-full p-5 md:p-6 flex flex-col justify-center relative overflow-hidden group hover:shadow-3xl transition-all duration-500 hover:scale-[1.02] cursor-pointer backdrop-blur-sm"
                        aria-label={`Benefit ${i + 1}: ${item.title}`}
                        style={{
                          background:
                            "linear-gradient(135deg, rgba(255,255,255,0.95) 0%, rgba(255,255,255,0.9) 50%, rgba(248,250,252,0.95) 100%)",
                        ...
                        <div className="relative z-10">
                          <div className="flex items-start gap-3 md:gap-4 mb-2 md:mb-3">
                            ...
                            <div className="flex-1 min-w-0">
                              <h3 className="text-slate-900 font-bold text-lg md:text-2xl leading-tight mb-0 group-hover:text-slate-800 transition-colors duration-300">
                                {item.title}
                              </h3>
      ...
      {/* CTA Button placed AFTER the whole section so it appears at the bottom.
          Wrapper uses the same blue background and a small negative top margin so CTA sits slightly overlapping the cards. */}
      <div className="bg-[#022150]" ref={ctaRef}>
        <div className="w-[90%] md:w-[80%] mx-auto max-w-7xl py-4 flex justify-center z-30 relative -mt-12 md:-mt-16">
          <button
