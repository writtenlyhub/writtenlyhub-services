...
const TestimonialScroller = ({ animationType = "continuous" }) => {
  ...
  const scroller1Items = [
    {
      type: "image",
      content: "https://i.ibb.co/yByVX299/1.png",
      alt: "Corporate handshake",
    },
    { type: "text", content: "150K+", subtext: "Clients" },
    {
      type: "image",
      content: "https://i.ibb.co/zVFxSjHM/4.png",
      alt: "Team collaboration",
    },
    {
      type: "text",
      content: "3x more",
      subtext: "inbound leads",
      description: "$36K+ additional revenue annually",
    },
    {
      type: "image",
      content: "https://i.ibb.co/Hp9Tfgxt/5.png",
      alt: "Business people",
    },
    {
      type: "text",
      content: "2x more",
      subtext: "inbound demos",
      description: "$25K+ additional MRR",
    },
    {
      type: "text",
      content: "30% higher",
  ...
  const scroller2Items = [
    ...
    {
      type: "image",
      content: "https://i.ibb.co/fVTVSL7L/9.png",
      alt: "Remote meeting",
    },
    {
      type: "text",
    ...
    },
    {
      type: "text",
      content: "12+",
      subtext: "qualified calls monthly",
      description: "$100K+ pipeline consistently",
    },
    {
      type: "image",
      content: "https://i.ibb.co/jt0CCgb/8.png",
      alt: "Content creator",
    },
    {
      type: "text",
    ...
    {
      type: "image",
      content: "https://i.ibb.co/dwSnTyh8/12.png",
      alt: "Creative team",
    },
    {
      type: "text",
  ...
  const scroller3Items = [
    ...
    {
      type: "image",
      content: "https://i.ibb.co/q32fqvFf/2.png",
      alt: "Business option",
    },
    {
      type: "text",
    ...
    {
      type: "image",
      content: "https://i.ibb.co/mdjFvfb/3.png",
      alt: "Office team",
    },
    {
      type: "text",
    ...
    {
      type: "image",
      content: "https://i.ibb.co/bgW0X4B5/10.png",
      alt: "Happy employees",
    },
  ];
  ...
  const duplicateItems = (items, times = 3) => {
    if (animationType === "scroll-triggered") {
      return [...items, ...items, ...items];
    }

    // For continuous mode, add unique IDs
    let cloned = [];
  ...
  useEffect(() => {
    const setupAnimations = () => {
      ...
      if (animationType === "continuous") {
        ...
        // Scroll-triggered animations (from second file) - THIS IS THE KEY CHANGE
        const moveAmount = isMobile ? 100 : 200;

        const scrollers = [
          { ref: scroller1Ref, direction: -1 },
          { ref: scroller2Ref, direction: 1 }, // This makes scroller2 move opposite direction
    ...
    return () => {
      ...
      if (animationType === "continuous") {
        gsap.killTweensOf([
          scroller1Ref.current,
          scroller2Ref.current,
          scroller3Ref.current,
        ]);
      } else {
        ScrollTrigger.getAll().forEach(trigger => trigger.kill());
      }
    ...
    >
      ...
      <div className="relative z-10 h-full flex flex-col justify-center space-y-4 md:space-y-6 w-full px-0">
        ...
        <div className="absolute top-4 right-4 z-20">
          {/* <div className="px-3 py-1 text-xs bg-white/10 text-white rounded backdrop-blur-sm">
        ...
        <div className="overflow-hidden">
          <div
            ref={scroller1Ref}
            className="flex w-max mx-auto transform-gpu will-change-transform"
          >
            {duplicateItems(scroller1Items).map((item, index) => (
              <ScrollerItem
                key={item.uid || `sc1-${index}`}
                item={item}
                index={index}
              />
            ))}
